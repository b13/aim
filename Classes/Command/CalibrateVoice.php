...
final class CalibrateVoice extends Command
{
    ...
    protected function configure(): void
    {
        $this
            ...
            ->addOption(
                ...
                null,
                InputOption::VALUE_NONE,
                'Save the fragment as active instead of inactive. Without this, a newly saved '
                . 'fragment waits in the Prompt Management module until a human activates it. '
                . 'Its text is derived from crawled page content and, once active, is composed '
                . 'into the system prompt of every subsequent AI request in the tree.',
            )
            ->addOption(
                'scope',
                null,
                InputOption::VALUE_REQUIRED,
                'Comma-separated capabilities the fragment applies to (' . implode(', ', self::scopeValues()) . '). Defaults to all of them.',
            )
            ->addOption(
                'no-inherit',
                null,
                InputOption::VALUE_NONE,
                'Apply the fragment to the root page only, instead of inheriting it to every subpage.',
            );
    }
    ...
    protected function execute(InputInterface $input, OutputInterface $output): int
    {
        ...
        $hidden = !(bool)$input->getOption('activate');
        $inheritToSubpages = !(bool)$input->getOption('no-inherit');
        ...
        foreach ($rootPageIds as $rootPageId) {
            ...
            if ($hidden) {
                $output->writeln(
                    '<comment>The fragment is inactive. Review it in the Prompt Management module '
                    . 'and activate it there before it is used.</comment>'
                );
            }
        }
     ...
     * @return list<string>
     */
    private static function scopeValues(): array
    {
        return array_map(static fn(PromptFragmentScope $scope): string => $scope->value, PromptFragmentScope::cases());
    }

    /**
     * @return list<string>
     */
    private function resolveScope(mixed $raw): array
    {
        if (!is_string($raw) || trim($raw) === '') {
            return self::scopeValues();
        }

        $requested = array_values(array_filter(array_map('trim', explode(',', $raw)), static fn(string $v): bool => $v !== ''));
        $unknown = array_diff($requested, self::scopeValues());
        if ($unknown !== []) {
            throw new \InvalidArgumentException(sprintf(
                'Unknown scope(s): %s. Known scopes: %s.',
                implode(', ', $unknown),
                implode(', ', self::scopeValues()),
            ), 1788040000);
        }
